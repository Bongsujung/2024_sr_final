...
if __name__ == "__main__":
    ...
    # 두 가지 특징량
    #feature_list = ['fbank', 'mfcc', 'mfcc_delta']
    feature_list = ['mfcc']
    ...
    for feature in feature_list:
        ...
        train_feat_scp = \
            './%s/train_per_500/feats.scp' % (feature)
        train_out_dir = \
            './%s/train_per_500' % (feature)
        ...
        for (feat_scp, out_dir) in \
            ...
            with open(feat_scp, mode='r') as file_feat:
                ...
                for i, line in enumerate(file_feat):
                    # 각 행에는 발화 ID, 특징량 파일 경로,
                    # 프레임 수, 차원 수가 공백으로 구분되어
                    # 기록됨
                    # split 함수를 사용해 공백 구분 행을
                    # 리스트형 변수로 변환
                    parts = line.split()
                    # 0번째가 발화 ID
