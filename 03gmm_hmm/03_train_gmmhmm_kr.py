...
# 
# 
# 병렬처리를 위해 구현된 hmmfunc_parallel.py에서 MonoPhoneHMM 클래스를 임포트
from hmmfunc_parallel import MonoPhoneHMM
...
if __name__ == "__main__":
    ...
    # 학습용 HMM 파일
    base_hmm = './exp_val/model_3state_1mix/0.hmm'

    # 훈련 데이터 특징량 리스트 파일
    feat_scp = \
        '../01compute_features/mfcc_delta/train_per_500/feats.scp'

    # 훈련 데이터의 라벨 파일
    label_file = \
        './exp/data/train_per_500/text_int'
    ...
    # 이 프로그램에서는 일부 발화만 사용
    num_utters = 17500 # 17500 # 1050 # 105829
    ...
    for m in range(mixup_time+1):
        ...
        for iter in range(num_iter):
            ...
            # 변경사항 : 병렬처리 train 함수 사용 
            hmm.train_parallel(feat_list, label_list, num_workers = int(os.cpu_count() / 5) )
